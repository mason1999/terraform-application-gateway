...
locals {
  ...
  trusted_root_certificates = {
    app1_mason_devops = {
      name                = "mkcert-root"
      key_vault_secret_id = "https://testkv00000.vault.azure.net/secrets/mkcert-root/XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
    }
  }
}
...
locals {
  ...
  backend_http_settings = {
    app_1_http_settings = {
      name     = "app-1-http-settings"
      port     = 80
      protocol = "Http"
    }
    app_1_https_settings = {
      name                           = "app-1-https-settings"
      port                           = 443
      protocol                       = "Https"
      host_name                      = "app1.masondevops.com"
      trusted_root_certificate_names = [local.trusted_root_certificates.app1_mason_devops.name]
    }
  }
  ...
  request_routing_rules = {
    request_routing_rule_1_http = {
      ...
      rule_type                  = "Basic"
      priority                   = 100
      backend_http_settings_name = local.backend_http_settings.app_1_http_settings.name
    ...
    request_routing_rule_1_https = {
      ...
      rule_type                  = "Basic"
      priority                   = 101
      backend_http_settings_name = local.backend_http_settings.app_1_https_settings.name
      backend_address_pool_name  = local.backend_address_pools.app_1_backend.name
    }
  }
}
